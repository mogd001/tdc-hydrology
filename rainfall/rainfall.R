...
rainfall <- get_data_collection(collection = "Rainfall",
                            method = "Total", interval = "1 hour", time_interval = "P7D") %>%   # from = "20230101", to = "Data end"
...
p <- r %>%
  ...
  scale_x_datetime(breaks = seq(min(r$datetime, na.rm =TRUE), max(r$datetime, na.rm =TRUE), by = "12 hours"), date_labels = "%Y%m%d-%H") +
