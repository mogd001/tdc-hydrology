...
for (month_year in month_years) {
  ...
  rainfall <- rainfall %>% 
    mutate(rainfall_total = if_else(rainfall_total <= 0, 0.0001, rainfall_total), # account for 0 rainfall months
           x = easting, 
           y = northing) 
  ...
  p5 <- ggmap(basemap, darken = c(0.6, "white")) +
    coord_cartesian() + 
    geom_sf(nelsontasman_wgs84, mapping = aes(), fill = NA, color = "black", inherit.aes = FALSE) + # note: geom_sf sets to correct aspect ratio
    geom_point(pts, mapping = aes(x = longitude, y = latitude, size = rainfall_total, color = rainfall_total_capped)) +
    geom_text(pts, mapping = aes(x = longitude, y = latitude, label = round(rainfall_total, 0)), color = "white", size = 1.2, alpha = 1.0) +
  ...
  p6 <- ggmap(basemap, darken = c(0.6, "white")) +
    ...
    geom_point(pts, mapping = aes(x = longitude, y = latitude),  size = 0.5, color = "black", shape = 3) +
